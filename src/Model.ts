...
export abstract class Model extends orm.BaseEntity {
     ...
     *
     * `initailize()` is a static method creating a new model instance very safely.
     *
     * Unlike `TypeORM.BaseEntity.create()` method, who can cause the critical runtime error by
     * ommitting essential variables, the `initialize()` does not permit ommitting the
     ...
     *
     * `createJoinQueryBuilder()` is a static method who returns not only the
     ...
     * Get column name.
     *
     * `getColumn()` is a static method returning the column name.
     *
     * Unlike writing the column name by yourself manually, who can cause the critical runtime
     * error by a mis-typing error, the `getColumn()` can prevent the mis-typing error in the
     * compilation level.
     *
     * The `getColumn()` even supports the table alias, therefore the table alias addicted
     ...
     *
     * `getWhereArguments()` is a static method returning arguments for the
     ...
     * `getWhereArguments()` can prevent the mis-typing error in the compilation level.
     *
     * The `getWhereArguments()` even supports the table alias, therefore the table alias
 ...
 */
type Creator<T extends Model> = _Creator<T> & typeof Model;
