...
>(
    ...
    string,
    Record<string, [Field.ValueType<T[Literal]>, Field.ValueType<T[Literal]>]>,
];
...
>(
    ...
    if (rest.length <= 2) {
        ...
        const [operator, param]: [
            Operator | "IN" | "NOT IN",
            Field.ValueType<T[Literal]> | null,
        ] = (() => {
            const [operator, param] =
        ...
        // IS NULL || IS-NOT-NULL
        if (param === null) {
            if (operator === "=") return [`${left} IS NULL`] as any;
            else if (operator === "!=" || operator === "<>")
                return [`${left} IS NOT NULL`] as any;
            else
                throw new InvalidArgument(
                    `Error on ${creator.name}.getColumn(): unable to bind null value for the ${operator} operator.`,
                );
        }
    ...
    return [
        `${left} BETWEEN ${
            typeof minimum === "function" ? minimum() : `:${from}`
        } AND ${typeof maximum === "function" ? maximum() : `:${to}`}`,
        {
            [from]: minimum,
            [to]: maximum,
        },
...
function _Decompose_parameter(param: any): any {
    ...
    else return _Decompose_entity(param);
}
...
function _Decompose_entity(param: any): any {
    if (
        ...
        if (param instanceof BelongsAccessorBase) param = param.id;
        else {
            const pkField: string | undefined = findRepository(
                param.constructor,
            ).metadata.primaryColumns[0]?.propertyName;
            if (pkField !== undefined) param = param[pkField];
        }
    }
    return param;
}
